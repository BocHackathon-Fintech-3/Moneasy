...
boc_api.post = function(url,data,headers,callback){
...
boc_api.get = function (url,headers,callback){
    var options = {
        url: boc_api.base_api_url+url,
        method: 'GET',
        headers: headers
...
boc_api.patch = function (url,data,headers,callback){
...
boc_api.fundAvailability = function(accountId,amount=100,bankId="abc",currency="EUR",currencyRate=60){
    return new Promise((resolve, reject) => {
            ...
            boc_api.post("/v1/payments/fundAvailability?client_id="+boc_api.client_id+"&client_secret="+boc_api.client_secret, data,headers,function(error, response, body) {
                if (error) {
                        reject(error);
                } else { 
...
boc_api.tokenStatus = function(token){
    ...
    boc_api.post("/oauth2/introspect",null,function(error, response, body) {
        if (error) {
            if(callback){
                callback(error,null)
            }
...
boc_api.get_login_url = function(subId){
    ...
    usrLoginUrl = boc_api.base_api_url+"/oauth2/authorize?response_type=code&scope=UserOAuth2Security&client_id="+boc_api.client_id+"&subscriptionid="+subId
    return usrLoginUrl;
}
